...
### Versioned packages (v1; additive-only)

New protocol work MUST land in versioned packages to allow long-term coexistence:

- `nds.common.v1`: `RequestContext`, `ErrorStatus`, `ErrorCategory` (v3+ primitives)
- `nds.identity.v1`: `PersonaId`, external account bindings
- `nds.event.v1`: `EventEnvelope`, `Cursor` (replay/resume semantics)
- `nds.ledger.v1`: `Money` (fixed-point), `Balance`, `CreateTransaction*` (contract shape)
- `nds.sync.v1`: subscribe/watch/replay/resume request/response shapes
- `nds.metadata.v1`: typed metadata primitives + controlled extensions
- `nds.memory.v1`: memory-proof data structures (hash/anchoring/redaction)

Non-negotiables:

- Do NOT retrofit v2 message semantics to v3 types (that is a breaking semantic change).
- Once published, v3 packages must follow additive-only evolution (never renumber fields; never change meaning/units).

## Core invariants (non-negotiable)

- **Protocol first**: specs define behavior; implementations follow.
- **Event-sourced state**: state is derived from events + projections (event sourcing).
...
| `nds.ledger.v1.Money` | `java.math.BigDecimal` | `decimal` | Fixed-point via `(units, nanos)`; SDK adapters MUST be deterministic (scale=9); rounding MUST be explicit |
...
- **Spec-first**: `.proto` is the canonical definition
...
```
nds-api/
├── spec/     # L0: Protocol (canonical schema)
├── java/     # L1: Java SDK
└── csharp/   # L1: C# SDK
```
