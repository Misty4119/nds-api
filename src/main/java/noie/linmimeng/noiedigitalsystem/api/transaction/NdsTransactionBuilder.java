...
/**
 * NDS Transaction Builder - 交易構建器
 * 
 * <p>用於構建不可變的交易事件對象。</p>
 * 
 * <p><b>⚠️ 重要：解決多態陷阱</b></p>
 * <p>由於 {@link NdsTransaction} 是繼承自 {@link NdsEvent} 的接口，
 * 不能直接使用 {@link noie.linmimeng.noiedigitalsystem.api.event.NdsEventBuilder} 構建
 * （因為 build() 返回的是 NdsEvent，無法轉型為 NdsTransaction）。
 * 因此必須使用專用的 {@link NdsTransactionBuilder}。</p>
 * 
 * @since 2.0.0
 */
