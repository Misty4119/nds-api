...
```kotlin
repositories {
    mavenCentral()  // Maven Central（發布中）
    // NDS 協議層儲存庫（目前使用）
...
repositories {
    mavenCentral()  // Maven Central（發布中）
    maven { url 'https://repo.repsy.io/mvn/linmimeng/releases' }  // 目前使用
}
...
- **目前狀態**：發布進行中
- **當前儲存庫**：[Repsy Maven Repository](https://repo.repsy.io/mvn/linmimeng/releases)
- **未來 Group ID**：`io.github.misty4119`（GitHub 命名空間）或 `com.linmimeng`（域名命名空間）
